...
int toolbarlng[MAX_TOOLBAR_ITEM] = {48, 46, 47, 49, 45, 44};

char *toolbaricon[MAX_TOOLBAR_ITEM] = {"go-first",
                                       "go-previous",
                                       "go-next",
                                       "go-last",
                                       "media-playback-stop",
...
int hotkeykey[MAX_HOTKEY_ITEM] = {13, 14, 15, 16, 53, 1};

char hotkeycommand[MAX_HOTKEY_ITEM][MAX_HOTKEY_COMMAND_LEN] = {"undo all\n",
                                                               "redo all\n",
                                                               "undo one\n",
                                                               "redo one\n",
                                                               "thinking stop\n",
...
char *hotkeynamelist[] = {
    "",         "F1",        "F2",          "F3",          "F4",           "F5",
    "F6",       "F7",        "F8",          "F9",          "F10",          "F11",
    "F12",      "Ctrl + Up", "Ctrl + Down", "Ctrl + Left", "Ctrl + Right", "Ctrl + 1",
    "Ctrl + 2", "Ctrl + 3",  "Ctrl + 4",    "Ctrl + 5",    "Ctrl + 6",     "Ctrl + 7",
    "Ctrl + 8", "Ctrl + 9",  "Ctrl + 0",    "Ctrl + A",    "Ctrl + B",     "Ctrl + C",
    ...
    "Ctrl + V", "Ctrl + W",  "Ctrl + X",    "Ctrl + Y",    "Ctrl + Z",     "Escape",
    NULL};
...
GdkPixbuf *draw_overlay_scaled(GdkPixbuf  *pb,
                               int         w,
                               int         h,
                               gchar      *text,
                               const char *color,
                               ...
                               float       scale)
{
    ...
    sprintf(format,
            "<span foreground='%s' weight='%s' size='%d' "
            "font_family='%s'>%%s</span>",
            color,
            weight,
            (int)(font_sizeboardtext * 1000 * scale),
            font_familyboardtext);
...
const char *winrate2colorstr(int winrate)
{
    ...
    hsv         hsv_color     = {(100 - winrate) * (180 / 100.0),
                                 (winrate <= 0 ? losssaturation
                                  : winrate >= 100
                                      ? winsaturation
...
void update_textpos()
{
    ...
    for (int i = 0; i < piecenum; i++) {
        sprintf(posstring + accumulatedOffset,
                "%c%d%n",
                movepath[i] % boardsize + 'a',
                boardsize - 1 - movepath[i] / boardsize + 1,
                &offset);
...
void make_move(int y, int x)
{
    ...
    piecenum++;
    if (piecenum == boardsize * boardsize)
        isgameover = 1;
...
void show_dialog_undo_warning_query(GtkWidget *window)
{
    ...
    dialog = gtk_message_dialog_new(GTK_WINDOW(window),
                                    ...
                                    "%s",
                                    language == 0
                                        ? "The operation will stop the calculation. Do you want "
                                          "to continue?"
                                        : _T(clanguage[12]));
...
void show_dialog_swap_query2(GtkWidget *window)
{
    ...
    dialog = gtk_dialog_new_with_buttons(msg,
                                         ...
                                         language == 0 ? "Stay with white" : _T(clanguage[108]),
                                         1,
                                         language == 0 ? "Swap" : _T(clanguage[109]),
                                         2,
                                         language == 0 ? "Add 2 more pieces" : _T(clanguage[110]),
                                         3,
                                         NULL);
...
void show_dialog_swap_info(GtkWidget *window)
{
    ...
    dialog = gtk_message_dialog_new(GTK_WINDOW(window),
                                    ...
                                    GTK_BUTTONS_OK,
                                    "%s",
                                    language == 0 ? "Swap" : _T(clanguage[15]));
    gtk_window_set_title(GTK_WINDOW(dialog), "Yixin");
    result        = gtk_dialog_run(GTK_DIALOG(dialog));
    isneedrestart = 1;
    if (computerside == 2) {
...
void show_dialog_timeout(GtkWidget *window)
{
    ...
    dialog = gtk_message_dialog_new(GTK_WINDOW(window),
                                    ...
                                    GTK_BUTTONS_OK,
                                    "%s",
                                    language == 0 ? "Time out!" : _T(clanguage[105]));
    gtk_window_set_title(GTK_WINDOW(dialog), "Yixin");
    result = gtk_dialog_run(GTK_DIALOG(dialog));
    gtk_widget_destroy(dialog);
}
...
void show_dialog_forbidden_info(GtkWidget *window)
{
    ...
    dialog = gtk_message_dialog_new(GTK_WINDOW(window),
                                    ...
                                    GTK_BUTTONS_OK,
                                    "%s",
                                    language == 0 ? "Forbidden Move" : _T(clanguage[98]));
    gtk_window_set_title(GTK_WINDOW(dialog), "Yixin");
    result = gtk_dialog_run(GTK_DIALOG(dialog));
    gtk_widget_destroy(dialog);
}
...
void show_dialog_illegal_opening(GtkWidget *window)
{
    ...
    dialog = gtk_message_dialog_new(GTK_WINDOW(window),
                                    ...
                                    GTK_BUTTONS_OK,
                                    "%s",
                                    language == 0 ? "Illegal Opening" : _T(clanguage[16]));
    gtk_window_set_title(GTK_WINDOW(dialog), "Yixin");
    result        = gtk_dialog_run(GTK_DIALOG(dialog));
    isneedrestart = 1;
    gtk_widget_destroy(dialog);
...
void show_dialog_boardtext(GtkWidget *window, int x, int y)
{
    ...
    dialog = gtk_dialog_new_with_buttons("Board Text",
                                         GTK_WINDOW(windowmain),
                                         GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
                                         _T(clanguage[121]),
                                         GTK_RESPONSE_REJECT,
    ...
    result = gtk_dialog_run(GTK_DIALOG(dialog));
    if (result == GTK_RESPONSE_REJECT || result == GTK_RESPONSE_ACCEPT) {
        ptext = gtk_entry_get_text(GTK_ENTRY(entry[0]));
        if (ptext[0] == '\0' || result == GTK_RESPONSE_REJECT) {
            // delete for empty text
...
gboolean show_dbdelall_query()
{
    ...
    dialog = gtk_message_dialog_new(GTK_WINDOW(windowmain),
                                    ...
                                    GTK_BUTTONS_YES_NO,
                                    "%s",
                                    language == 0 ? "Are your sure to delete all branches?"
                                                  : _T(clanguage[116]));
    ...
    if (!isthinking && event->type == GDK_BUTTON_PRESS) {
        if (event->button == 1 || event->button == 2) {
            ...
            if (x >= 0 && x < boardsize && y >= 0 && y < boardsize && !isgameover) {
                if (usedatabase && (isctrlpressed || event->button == 2) && board[y][x] == 0
                    && (piecenum % 2 == 1 || !forbid[y][x])) {
                ...
                else if (!refreshboardflag
                         && ((specialrule != 1 && specialrule != 3) || piecenum >= 3
                             || piecenum == 0)
                         && (((computerside & 1) && piecenum % 2 == 0)
                             || ((computerside & 2) && piecenum % 2 == 1))) {
                    ...
                    if (specialrule == 3 && refreshboardflag == 1 && computerside != 2) {
                        ...
                        show_dialog_forbidden_info(widget);
                             ...
                             && (piecenum % 2 == 1 || !forbid[y][x] || refreshboardflag)) {
                        int i;
                        int flag = 0;
                        ...
                        if (specialrule == 2 && piecenum == 1 && computerside != 0) {
                            ...
                            // the condition for swapping under `swap after 1st
                            // move' rule
                            if ((((_x == 2 && _y == 3) || (_x == 3 && _y == 2)) && rand() % 2 == 1)
                                || (_x > 1 && _y > 1 && _x + _y > 5)) {
                                show_dialog_swap_info(widget);
                            }
                        }
                        if (specialrule == 3 && piecenum == 3 &&
                            /*computerside != 0 &&*/ computerside != 1) {
                            // check whether the current opening is one of the
                            // 26 standard openings
                            if (movepath[0] % boardsize == boardsize / 2
                                && movepath[0] / boardsize == boardsize / 2
                                && movepath[1] % boardsize <= boardsize / 2 + 1
                                && movepath[1] % boardsize >= boardsize / 2 - 1
                                && movepath[1] / boardsize <= boardsize / 2 + 1
                                && movepath[1] / boardsize >= boardsize / 2 - 1
                                && movepath[2] % boardsize <= boardsize / 2 + 2
                                && movepath[2] % boardsize >= boardsize / 2 - 2
                                && movepath[2] / boardsize <= boardsize / 2 + 2
                                && movepath[2] / boardsize >= boardsize / 2 - 2) {
                                // do nothing
                        ...
                        if (specialrule == 3 && piecenum == 3 && computerside != 0
                            && computerside != 1) {
                            timerhumanincrement += increment;
                        ...
                        if (specialrule == 3 && piecenum == 4 && computerside != 0
                            && computerside != 2) {
                        ...
                        if (specialrule == 3 && piecenum >= 5 && piecenum <= 5 + move5N - 1
                            && computerside != 0 && computerside != 1) {
                        ...
                        if (specialrule == 3 && piecenum == 5 + move5N - 1 && computerside != 0
                            && computerside != 1 && refreshboardflag2 == 0) {
                            refreshboardflag  = 0;
                            refreshboardflag2 = 1;
                            isneedrestart     = 1;
                            timerhumanincrement += increment;
...
void show_dialog_settings(GtkWidget *widget, gpointer data)
{
    ...
    gtk_notebook_append_page(GTK_NOTEBOOK(notebook),
                             notebookvbox[0],
                             gtk_label_new(language == 0 ? "Level" : _T(clanguage[18])));
    notebookvbox[1] = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
    gtk_notebook_append_page(GTK_NOTEBOOK(notebook),
                             notebookvbox[1],
                             gtk_label_new(language == 0 ? "Style" : _T(clanguage[19])));
    ...
    radiolevel[0] =
        gtk_radio_button_new_with_label(NULL, language == 0 ? "Unlimited Time" : _T(clanguage[36]));
    radiolevel[1] =
        gtk_radio_button_new_with_label(gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiolevel[0])),
                                        language == 0 ? "Custom Level" : _T(clanguage[32]));
    radiolevel[2] =
        gtk_radio_button_new_with_label(gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiolevel[1])),
                                        language == 0 ? "Predefined Level" : _T(clanguage[28]));

    g_signal_connect(G_OBJECT(radiolevel[0]),
                     "toggled",
                     G_CALLBACK(show_dialog_settings_custom_entry),
                     (gpointer)0);
    g_signal_connect(G_OBJECT(radiolevel[1]),
                     "toggled",
                     G_CALLBACK(show_dialog_settings_custom_entry),
                     (gpointer)1);
    g_signal_connect(G_OBJECT(radiolevel[2]),
                     "toggled",
                     G_CALLBACK(show_dialog_settings_custom_entry),
                     (gpointer)2);
    ...
    gtk_box_pack_start(GTK_BOX(hbox[11]),
                       gtk_label_new(language == 0 ? "Fast" : _T(clanguage[29])),
                       FALSE,
                       FALSE,
                       3);
    gtk_box_pack_start(GTK_BOX(hbox[11]), scalelevel, FALSE, FALSE, 3);
    gtk_box_pack_start(GTK_BOX(hbox[11]),
                       gtk_label_new(language == 0 ? "Slow" : _T(clanguage[30])),
                       FALSE,
                       FALSE,
                       3);
    gtk_box_pack_start(GTK_BOX(hbox[11]), gtk_label_new("     "), FALSE, FALSE, 3);
    ...
    GtkAdjustment *adjustment_hashsize =
    ...
    radiovcthread[1] = gtk_radio_button_new_with_label(
        gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiovcthread[0])),
        language == 0 ? "Check VCT" : _T(clanguage[86]));
    radiovcthread[2] = gtk_radio_button_new_with_label(
        gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiovcthread[1])),
        language == 0 ? "Check VC2" : _T(clanguage[88]));
    ...
    gtk_box_pack_start(GTK_BOX(hbox[1]),
                       gtk_label_new(language == 0 ? "Rash" : _T(clanguage[37])),
                       FALSE,
                       FALSE,
                       3);
    gtk_box_pack_start(GTK_BOX(hbox[1]), scalecaution, FALSE, FALSE, 3);
    gtk_box_pack_start(GTK_BOX(hbox[1]),
                       gtk_label_new(language == 0 ? "Cautious" : _T(clanguage[38])),
                       FALSE,
                       FALSE,
                       3);

    gtk_box_pack_start(GTK_BOX(notebookvbox[1]),
                       gtk_label_new(language == 0 ? "Additional Threat Check in Global Search:"
                                                   : _T(clanguage[31])),
                       FALSE,
                       FALSE,
                       3);
    ...
    if (maxthreadnum > 1) {
        ...
        gtk_box_pack_start(GTK_BOX(hbox[2]),
                           gtk_label_new(language == 0 ? "Number of Threads" : _T(clanguage[39])),
                           FALSE,
                           FALSE,
                           3);
    ...
    gtk_box_pack_start(GTK_BOX(hbox[3]),
                       gtk_label_new(language == 0 ? "Hash Size" : _T(clanguage[41])),
                       FALSE,
                       FALSE,
                       3);
    ...
    case GTK_RESPONSE_ACCEPT:
        if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(radiolevel[1]))) {
            ...
            if (is_integer(ptext)) {
                ...
                if (maxdepth > boardsize * boardsize)
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        ...
        if ((filename[nl - 3] == 'P' || filename[nl - 3] == 'p')
            ...
            if ((in = fopen(filename, "r")) != NULL) {
                int i, num;
                new_game(NULL, NULL);
                fscanf(in, "%*d");  // TODO: use boardsize?
                fscanf(in, "%*d");  // TODO: use boardsize or inforule?
                /*
                fscanf(in, "%d", &inforule);
                switch(inforule)
                {
                    case 0:
                gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(menuitemrule1),
                TRUE); break; case 1:
                gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(menuitemrule2),
                TRUE); break; case 2:
                gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(menuitemrule3),
                TRUE); break; case 3:
                gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(menuitemrule4),
                TRUE); break;
                }
                */
            ...
            if ((in = fopen(filename, "r")) != NULL) {
                ...
                new_game(NULL, NULL);
                fscanf(in,
                       "%*[^\n]%*c");  // TODO: use boardsize, boardsize, etc.?
...
void show_dialog_move5N(GtkWidget *widget, gpointer data)
{
    ...
    dialog = gtk_dialog_new_with_buttons("Number of 5th Moves",
                                         GTK_WINDOW(windowmain),
                                         GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
                                         _T(clanguage[117]),
                                         GTK_RESPONSE_ACCEPT,
...
void show_dialog_size(GtkWidget *widget, gpointer data)
{
    ...
    label = gtk_label_new(language == 0 ? "Board Size (5 ~ 22):" : _T(clanguage[43]));
...
void show_dialog_custom_toolbar(GtkWidget *widget, gpointer data)
{
    ...
    dialog = gtk_dialog_new_with_buttons(_T(clanguage[119]),
                                         windowmain,
                                         GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
                                         _T(clanguage[117]),
                                         GTK_RESPONSE_ACCEPT,
    ...
    for (GList *iter = icon_list; iter != NULL; iter = iter->next) {
        ...
        if (gtk_icon_theme_has_icon(icon_theme, icon_name)) {
            GdkPixbuf *pixbuf =
                gtk_icon_theme_load_icon(icon_theme, icon_name, GTK_ICON_SIZE_BUTTON, 0, NULL);
            if (pixbuf) {
                GtkTreeIter iter;
                icon_count++;
    ...
    gtk_text_buffer_insert(GTK_TEXT_BUFFER(buffercommand),
                           &end,
                           toolbarcommand[GPOINTER_TO_INT(data)],
                           strlen(toolbarcommand[GPOINTER_TO_INT(data)]));
...
void show_dialog_custom_hotkey(GtkWidget *widget, gpointer data)
{
    ...
    dialog = gtk_dialog_new_with_buttons("Custom Hotkey",
                                         windowmain,
                                         GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
                                         _T(clanguage[117]),
                                         GTK_RESPONSE_ACCEPT,
    ...
    gtk_text_buffer_insert(GTK_TEXT_BUFFER(buffercommand),
                           &end,
                           hotkeycommand[GPOINTER_TO_INT(data)],
                           strlen(hotkeycommand[GPOINTER_TO_INT(data)]));
...
void execute_command(gchar *command)
{
    ...
    if (yixin_strnicmp(command, "command on", 10) == 0) {
        ...
        do {
            ...
            y--;
            if (s < 0 || s > 1 || x < 0 || x >= boardsize || y < 0 || y >= boardsize)
                break;
    ...
    else if (yixin_strnicmp(command, "balance", 7) == 0
             && (command[7] == '1' || command[7] == '2')) {
        gchar _command[80];
        int   s;
        int   t;
        t = command[7] - '0';
        ...
        if (command[5] == 0 || command[5] == '\n' || command[5] == '\r' || command[5] == 'o') {
            const char *dbfilename =
                show_dialog_dbselect(NULL, windowmain, command[5] == 'o' ? 0 : 1);
            if (dbfilename) {
...
void save_setting()
{
    ...
    if ((out = fopen("settings.txt", "w")) != NULL) {
        fprintf(out, "%d\t;board size (10 ~ %d)\n", rboardsize, MAX_SIZE);
        fprintf(out, "%d\t;language (0: English, 1,2,...: custom)\n", rlanguage);
        fprintf(out,
                "%d\t;rule (0: freestyle, 1: standard, 2: free renju, 3: swap "
                "after 1st move, 5: soosorv, 6: swap-2)\n",
        ...
        fprintf(out,
                "%d\t;level (0: unlimited time 1: custom level 2-12: predefined level)\n",
                levelchoice);
        ...
        fprintf(out,
                "%d\t;toolbar style (0: only icon, 1: both icon and words, 2: "
                "both with horizontally stacked)\n",
                showtoolbarboth);
        ...
        fprintf(out, "%s;Board Text Font\n", fontname[0]);
        fprintf(out, "%s;Text Log Font\n", fontname[1]);
        fprintf(out, "%s;Database Comment Font\n", fontname[2]);
        fprintf(out, "%s;Database Comment Font\n", fontname[3]);
        fclose(out);
    }
...
void create_windowclock()
{
    ...
    // gtk_label_set_markup(GTK_LABEL(playerlabel[0]),
    // "<big><b>Yixin</b></big>");

    playerlabel[1] = gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(playerlabel[1]), "<big><b>Human</b></big>");
    // gtk_label_set_markup(GTK_LABEL(playerlabel[1]), "<big><b>Lin
    // Shu-Hsuan</b></big>"); gtk_label_set_markup(GTK_LABEL(playerlabel[1]),
    // "<big><b>Rudolf Dupszki</b></big>");
...
gboolean create_menu_proxy(GtkToolItem *tool_item, gpointer user_data)
{
    ...
    gtk_css_provider_load_from_data(provider, "menuitem { padding: 4px 0px; }", -1, NULL);
    ...
    GtkWidget *menu_image =
        gtk_image_new_from_icon_name(toolbaricon[i] ? toolbaricon[i] : "image-missing",
                                     GTK_ICON_SIZE_MENU);
    ...
    g_signal_connect(G_OBJECT(menu_item),
                     "activate",
                     G_CALLBACK(toolbar_function),
                     GINT_TO_POINTER(i));
    gtk_tool_item_set_proxy_menu_item(tool_item,
                                      g_strdup_printf("overflow-proxy-%d", i),
                                      menu_item);
...
void create_windowmain()
{
    ...
    GtkWidget *menuitemnewgame, *menuitemload, *menuitemsave, *menuitemrule, *menuitemsize,
        *menuitemquit, *menuitemrule1, *menuitemrule2, *menuitemrule3, *menuitemrule4,
        *menuitemrule5, *menuitemrule6, *menuitemrule7, *menuitemrule8;
    ...
    g_signal_connect(G_OBJECT(eventbox_board),
                     "button_press_event",
                     G_CALLBACK(on_button_press_windowmain),
                     NULL);
    ...
    for (i = 0; i < boardsize; i++) {
        for (j = 0; j < boardsize; j++) {
            if (i == 0 && j == 0)
                ...
                imgtypeboard[i][j] = 9;
            else if (i == boardsize / 4 && j == boardsize / 4)
                imgtypeboard[i][j] = 9;
            else if (i == boardsize / 4 && j == boardsize - 1 - boardsize / 4)
                imgtypeboard[i][j] = 9;
            else if (i == boardsize - 1 - boardsize / 4 && j == boardsize / 4)
                imgtypeboard[i][j] = 9;
            else if (i == boardsize - 1 - boardsize / 4 && j == boardsize - 1 - boardsize / 4)
                imgtypeboard[i][j] = 9;
            else
                imgtypeboard[i][j] = 0;
        }
    ...
    for (i = 0; i < boardsize; i++) {
        ...
        gtk_grid_attach(GTK_GRID(tableboard), labelboard[0][i], boardsize, i, 1, 1);
    }
    for (i = 0; i < boardsize; i++) {
        char tlabel[3];
        sprintf(tlabel, "%c", 'A' + i);
        labelboard[1][i] = gtk_label_new(tlabel);
        gtk_grid_attach(GTK_GRID(tableboard), labelboard[1][i], i, boardsize, 1, 1);
    }
    for (i = 0; i < boardsize; i++) {
        for (j = 0; j < boardsize; j++) {
            if (imgtypeboard[i][j] <= 8)
    ...
    menuitemnumeration =
        gtk_check_menu_item_new_with_label(language == 0 ? "Numeration" : _T(clanguage[69]));
    ...
    menuitemdatabase =
        gtk_check_menu_item_new_with_label(language == 0 ? "Use Database" : _T(clanguage[103]));
    menuitemdbreadonly = gtk_check_menu_item_new_with_label(language == 0 ? "Database Readonly"
                                                                          : _T(clanguage[112]));
    menuitemboardtext =
    ...
    menuitemrule1 =
        gtk_radio_menu_item_new_with_label(NULL,
                                           language == 0 ? "Freestyle Gomoku" : _T(clanguage[76]));
    menuitemrule2 = gtk_radio_menu_item_new_with_label(
        gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(menuitemrule1)),
        language == 0 ? "Standard Gomoku" : _T(clanguage[77]));
    menuitemrule3 = gtk_radio_menu_item_new_with_label(
        gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(menuitemrule1)),
        language == 0 ? "Free Renju" : _T(clanguage[78]));
    menuitemrule5 = gtk_radio_menu_item_new_with_label(
        gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(menuitemrule1)),
        language == 0 ? "Swap After First Move" : _T(clanguage[80]));
    menuitemrule6 = gtk_radio_menu_item_new_with_label(
        gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(menuitemrule1)),
        language == 0 ? "Swap2" : _T(clanguage[100]));
    menuitemrule8 = gtk_radio_menu_item_new_with_label(
        gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(menuitemrule1)),
        language == 0 ? "Soosorv-8 Opening Rule" : _T(clanguage[102]));
    ...
    for (i = 1; i < 16; i++) {
        ...
        if ((in = fopen(s, "r")) != NULL) {
            ...
            fclose(in);
            menuitemcustomlng[i] = gtk_radio_menu_item_new_with_label(
                gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(menuitemenglish)),
                _T(lnglis[i]));
        }
    }
    ...
    menuitemcomputerplaysblack = gtk_check_menu_item_new_with_label(
        language == 0 ? "Computer Plays Black" : _T(clanguage[81]));
    menuitemcomputerplayswhite = gtk_check_menu_item_new_with_label(
        language == 0 ? "Computer Plays White" : _T(clanguage[82]));
    menuitemchecktimeout =
        gtk_check_menu_item_new_with_label(language == 0 ? "Check Timeout" : _T(clanguage[99]));
    if (computerside & 1)
    ...
    g_signal_connect(G_OBJECT(menuitemdbreadonly),
                     "activate",
    ...
    g_signal_connect(G_OBJECT(menuitemabout),
                     "activate",
                     G_CALLBACK(show_dialog_about),
                     GTK_WINDOW(windowmain));
    g_signal_connect(G_OBJECT(menuitemsettings),
                     "activate",
                     G_CALLBACK(show_dialog_settings),
                     GTK_WINDOW(windowmain));
    ...
    g_signal_connect(G_OBJECT(menuitemcomputerplaysblack),
                     "activate",
                     G_CALLBACK(change_side),
                     (gpointer)1);
    g_signal_connect(G_OBJECT(menuitemcomputerplayswhite),
                     "activate",
                     G_CALLBACK(change_side),
                     (gpointer)2);
    g_signal_connect(G_OBJECT(menuitemchecktimeout),
                     "activate",
                     G_CALLBACK(change_timeoutcheck),
                     NULL);
    g_signal_connect(G_OBJECT(menuitemenglish),
                     "activate",
                     G_CALLBACK(change_language),
                     (gpointer)0);
    for (i = 1; i < 16; i++) {
        if (menuitemcustomlng[i] != NULL)
            g_signal_connect(G_OBJECT(menuitemcustomlng[i]),
                             "activate",
                             G_CALLBACK(change_language),
                             GINT_TO_POINTER(i));
    }
    ...
    for (i = 0; i < toolbarnum; ++i) {
        ...
        g_signal_connect(G_OBJECT(tool_btn),
                         "clicked",
                         G_CALLBACK(toolbar_function),
                         GINT_TO_POINTER(i));
        g_signal_connect(G_OBJECT(tool_btn),
    ...
    gtk_widget_set_size_request(scrolledtextdbcomment,
                                (int)(hdpiscale * 400),
                                (int)(hdpiscale * 100));
    ...
    gchar *css_data           = g_strdup_printf("textview { font-family: '%s'; font-size: %dpt; }",
                                      font_familydbcomment ? font_familydbcomment : "Sans",
                                      font_sizedbcomment > 0 ? font_sizedbcomment : 10);
    ...
    gtk_style_context_add_provider(gtk_widget_get_style_context(textdbcomment),
                                   GTK_STYLE_PROVIDER(css_providerdbcomment),
                                   GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
    ...
    gchar *css_data2 = g_strdup_printf("textview { font-family: '%s'; font-size: %dpt; }",
                                       font_familylog ? font_familylog : "Sans",
                                       font_sizelog > 0 ? font_sizelog : 10);
    ...
    gtk_style_context_add_provider(gtk_widget_get_style_context(textlog),
                                   GTK_STYLE_PROVIDER(css_providerlog),
                                   GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
    ...
    gtk_widget_set_size_request(
        scrolledtextlog,
        (int)(hdpiscale * 400),
        max(0, size * boardsize - (int)(hdpiscale * 200) - (toolbarpos == 1 ? 50 : 0)));
    ...
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolledtextpos),
                                   GTK_POLICY_NEVER,
                                   GTK_POLICY_ALWAYS);
    ...
    gchar *css_data3 = g_strdup_printf("textview { font-family: '%s'; font-size: %dpt; }",
                                       font_familypos ? font_familypos : "Sans",
                                       font_sizepos > 0 ? font_sizepos : 10);
    ...
    gtk_style_context_add_provider(gtk_widget_get_style_context(textpos),
                                   GTK_STYLE_PROVIDER(css_providerpos),
                                   GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
    ...
    gtk_button_set_image(GTK_BUTTON(btncopypos),
                         gtk_image_new_from_icon_name("edit-copy", GTK_ICON_SIZE_BUTTON));
    gtk_button_set_image(GTK_BUTTON(btnsetpos),
    ...
    if (toolbarpos == 0) {
        ...
        gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(toolbar_scrolled),
                                       GTK_POLICY_NEVER,
                                       GTK_POLICY_AUTOMATIC);
        int max_toolbar_height = max(200, size * boardsize - 150);
...
gboolean iochannelout_watch(GIOChannel *channel, GIOCondition cond, gpointer data)
{
    ...
    do {
        ...
        if (strncmp(string, "OK", 2) == 0) {
            ...
            else if (*p == 'O')  //"ONE"
            {
                ...
                const char *boundtext = bound == 3   ? "exact"
                                        : bound == 2 ? "lower"
                                        : bound == 1 ? "upper"
                                                     : "none";
            ...
            else if (*p == 'T')  //"TEXT"
            {
                ...
                if (*t == '"') {
                    ...
                    while (t[len] != '"') {
                        if (t[len] == '\0') {
                            gtk_text_buffer_insert(GTK_TEXT_BUFFER(buffertextdbcomment),
                                                   &end,
                                                   t,
                                                   len);
                            g_free(rawstring);
                ...
                if (strncmp(p, "START", 5) == 0) {
                    ...
                    if (loadingdialog == NULL) {
                        loadingdialog = gtk_message_dialog_new(
                            ...
                            GTK_BUTTONS_NONE,
                            "%s %s ......",
                            language == 0 ? "Loading database from" : _T(clanguage[120]),
                            _T(rawp + 11));
    ...
    return TRUE; /* TRUE means that the recalling function still runs later,
                    while FALSE indicates it no longer runs */
}
...
void load_setting(int def_boardsize, int def_language, int def_toolbar)
{
    ...
    if ((in = fopen("settings.txt", "r")) != NULL) {
        ...
        if (boardsize > MAX_SIZE || boardsize < 5)
            boardsize = 15;
        ...
        if (maxdepth < 2 || maxdepth > boardsize * boardsize)
            maxdepth = boardsize * boardsize;
        ...
        fgets(s, sizeof(s), in);  // skip the new line
        for (int i = 0; i < 4; i++) {
            read_str_from_file(in, fontname[i], sizeof(fontname[i]));
        }
...
void load_engine()
{
    ...
    // g_io_add_watch_full(iochannelout, G_PRIORITY_HIGH, G_IO_IN | G_IO_HUP,
    // (GIOFunc)iochannelout_watch, NULL, NULL);
...
int main(int argc, char **argv)
{
    static GOptionEntry options[] = {
        {"size", 's', 0, G_OPTION_ARG_INT, NULL, "Board size to use", "Integer"},
        {"lang", 'l', 0, G_OPTION_ARG_INT, NULL, "language", "Integer"},
        {"toolbar", '\0', 0, G_OPTION_ARG_INT, NULL, "Tool bar style", "Integer"},
        {NULL},
    };
    GError *error     = NULL;
    gint    boardsize = -1;
